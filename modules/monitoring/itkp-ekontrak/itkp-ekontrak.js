(function () {
  ...
  const EKONTRAK_DETAIL_PAGE_SIZE = 10;
  const EKONTRAK_MAX_ITKP = 5;

  window.__moduleInit = function ({ container }) {
    const root = container.querySelector('.itkp-ekontrak-page');
    if (!root) return null;
    ...
    on(EL.btnExportDetail, 'click', () => {
      exportRows(getFilteredRawRows(), 'detail-ekontrak-semua-filter.xlsx');
    });

    on(EL.btnExportCurrentDetail, 'click', () => {
      exportRows(
        state.selectedDetailRows,
        `detail-ekontrak-${slugify(state.selectedOpd || 'semua-opd')}.xlsx`
      );
    });
    ...
    on(EL.btnShowMaxList, 'click', () => {
      const items = state.filteredRekap
        .filter(row => row.nilai_itkp >= EKONTRAK_MAX_ITKP)
        .map(row => row.satuan_kerja);

      openModal(
        'Daftar OPD Capai Target Max',
        `OPD yang sudah mencapai nilai ITKP maksimal eKontrak yaitu ${EKONTRAK_MAX_ITKP} poin.`,
        items
      );
    });
    ...
    on(EL.btnShowZeroList, 'click', () => {
      ...
      openModal(
        ...
        'OPD yang indikator pemanfaatan eKontrak-nya masih 0.',
        items
      );
    });
    ...
    return () => {
      ...
      listeners.forEach(off => {
        try {
          ...
          console.warn('Gagal melepas listener eKontrak:', err);
        }
      });
    ...
    async function initMonitoring(useOverlay = false) {
      ...
      try {
        ...
        if (elapsed < EKONTRAK_MIN_LOADING_MS) {
          await wait(EKONTRAK_MIN_LOADING_MS - elapsed);
        }
    ...
    function renderRekapTable() {
      ...
      const totalPages = Math.max(1, Math.ceil(totalItems / EKONTRAK_PAGE_SIZE));
      state.rekapPage = Math.min(state.rekapPage, totalPages);

      const start = (state.rekapPage - 1) * EKONTRAK_PAGE_SIZE;
      const end = start + EKONTRAK_PAGE_SIZE;
      ...
      if (!pageRows.length) {
        ...
        EL.rekapTableBody.innerHTML = pageRows.map((row, index) => `
          <tr>
            ...
            <td>${formatInt(row.total_paket_aktif)}</td>
            <td>${formatInt(row.total_paket_selesai)}</td>
    ...
    function renderDetailTable() {
      ...
      const totalItems = rows.length;
      const totalPages = Math.max(1, Math.ceil(totalItems / EKONTRAK_DETAIL_PAGE_SIZE));
      state.detailPage = Math.min(state.detailPage, totalPages);

      const start = (state.detailPage - 1) * EKONTRAK_DETAIL_PAGE_SIZE;
      const end = start + EKONTRAK_DETAIL_PAGE_SIZE;
      const pageRows = rows.slice(start, end);

      if (EL.detailTitle) {
        EL.detailTitle.textContent = state.selectedOpd
          ? `Detail Paket eKontrak - ${state.selectedOpd}`
          : 'Detail Paket eKontrak';
      }
  ...
  function normalizeScoreRows(rows) {
    return rows.map(row => ({
      satuan_kerja: pick(row, ['satuan_kerja', 'satker']),
      total_paket_aktif: toNumber(pick(row, ['total_paket_aktif', 'paket_aktif'])),
      total_paket_selesai: toNumber(pick(row, ['total_paket_selesai', 'paket_selesai'])),
  ...
  function renderItkpBadge(value) {
    const klass = value >= EKONTRAK_MAX_ITKP ? 'badge-green' : value > 0 ? 'badge-yellow' : 'badge-red';
